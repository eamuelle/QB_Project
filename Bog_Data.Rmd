...
```{r}
#generate ssad for multiple randomly chosen OTU
...
```
```{r}
#beta diversity of bog communities
...
plot(bog.pcoa$points[,1], bog.pcoa$points[,2],
     ...
     pch = 16, cex = 3.0, type = "n", cex.lab = 1.2, cex.axis = 1.2, axes = FALSE, xlim = c(-0.4, 0.5))
...
while(num - 1 < nrow(metadata_JUL07)){
...
#dbRDA from metadata only lats + lons
bog.dbrda <- dbrda(otu_JUL07~ lats + lons + Temperature, as.data.frame(metadata_JUL07), dist = "bray")
...
plot(scores(bog.dbrda, display = "wa"), xlim = c(-1.3, 1.5), ylim = c(-1.5, 2.7), xlab = paste("dbRDA 1 (", dbrda.explainvar1, "%)", sep = ""), ylab = paste("dbRDA 2 (", dbrda.explainvar2, "%)", sep = ""), pch = 16, cex = 2.0, type = "n", cex.lab = 1.5, cex.axis = 1.2, axes = FALSE)
...
```{r}
#Relationship between environmental distance, geographic distance and community distance
